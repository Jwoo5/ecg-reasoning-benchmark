...
1. Create a new Python file in the `models/` directory, and define a new model class that extends `BaseModel` class defined in `models/model.py`.
2. This new model class should be decorated with `@register_model(model_name)` to register the model with a unique name for loading the model, where `model_name` is the unique name for the model (e.g., `my_model`).
